...
class JettyWebSocketRequiredRegister {
    ...
    void register(FeatureUtils featureUtils) throws NoSuchMethodException {
        Class<?> byteBufferDecoderClass = featureUtils.loadClass("org.eclipse.jetty.ee11.websocket.jakarta.common.decoders.ByteBufferDecoder");
        if (byteBufferDecoderClass != null) RuntimeReflection.register(byteBufferDecoderClass.getConstructor());
        Class<?> stringDecoderClass = featureUtils.loadClass("org.eclipse.jetty.ee11.websocket.jakarta.common.decoders.StringDecoder");
        if (stringDecoderClass != null) featureUtils.registerReflection(stringDecoderClass.getConstructor());
        Class<?> decodedBinaryMsgSinkClass = featureUtils.loadClass("org.eclipse.jetty.ee11.websocket.jakarta.common.messages.DecodedBinaryMessageSink");
        if (decodedBinaryMsgSinkClass != null) {
            featureUtils.registerReflection(decodedBinaryMsgSinkClass.getConstructors());
            for (Method method : decodedBinaryMsgSinkClass.getDeclaredMethods()) {
                if (method.getName().equals("onWholeMessage") || method.getName().equals("onMessage")) {
                    featureUtils.registerReflection(method);
                    break;
                }
            }
        }
        Class<?> decodedTextMsgSinkClass = featureUtils.loadClass("org.eclipse.jetty.ee11.websocket.jakarta.common.messages.DecodedTextMessageSink");
        if (decodedTextMsgSinkClass != null) {
            featureUtils.registerReflection(decodedTextMsgSinkClass.getConstructors());
            for (Method method : decodedTextMsgSinkClass.getDeclaredMethods()) {
