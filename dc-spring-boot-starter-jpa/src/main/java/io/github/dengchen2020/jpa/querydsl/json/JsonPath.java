...
public class JsonPath<T> extends SimpleExpression<T> implements JsonObjectOperation, JsonArrayOperation {
    ...
    public BooleanExpression contains(String json) {
        ...
        return Expressions.booleanTemplate("sql('? @> ?::jsonb',{0},{1})", path, json).isTrue();
    }
    ...
    public BooleanExpression contains(Map<String, Object> json) {
        return Expressions.booleanTemplate("sql('? @> ?::jsonb',{0},{1})", path, JsonUtils.toJson(json)).isTrue();
    }
    ...
    public BooleanExpression contains(ObjectNode json) {
        return Expressions.booleanTemplate("sql('? @> ?::jsonb',{0},{1})", path, json.toString()).isTrue();
    }
    ...
    public BooleanExpression containsKey(String key) {
        return Expressions.booleanTemplate("sql('jsonb_exists(?,?)',{0},{1})", path, key).isTrue();
    }

    public JsonObjectTemplate getObject(String key){
        return new JsonObjectTemplate("sql('? -> ?',{0},{1})", path, key);
    }

    public JsonObjectTemplate getObject(int index){
        return new JsonObjectTemplate("sql('? -> ?',{0},{1})", path, index);
    }

    public JsonArrayTemplate getArray(String key){
        return new JsonArrayTemplate("sql('? -> ?',{0},{1})", path, key);
    }

    public JsonArrayTemplate getArray(int index){
        return new JsonArrayTemplate("sql('? -> ?',{0},{1})", path, index);
    }

    public JsonTemplate get(String key){
        return new JsonTemplate("jsonb_get_str({0},{1})", path, key);
    }

    @Override
    public JsonTemplate get(int index) {
        return new JsonTemplate("jsonb_get_str({0},{1})", path, index);
    }

    public JsonObjectTemplate getObject(String... pathArr){
        return new JsonObjectTemplate("sql('? #> ?',{0},{1})", path, pathArr);
    }

    public JsonTemplate get(String... pathArr) {
        return new JsonTemplate("jsonb_get_str_by_patharr({0},{1})", path, pathArr);
    }
}
