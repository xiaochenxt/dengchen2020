...
import io.github.dengchen2020.jpa.querydsl.QuerydslUtils;
...
public class QuerydslJpaRepositoryExecutor<T> implements QuerydslJpaRepository<T>, QuerydslPagingJpaRepository<T> {
    ...
    @Override
    public <R> JPAQuery<R> selectBean(Class<? extends R> type, Expression<?>... exprs) {
        return select(QuerydslUtils.bean(type, path, exprs));
    }

    @Override
    public <R extends Record> JPAQuery<R> selectRecord(Class<? extends R> type, Expression<?>... exprs) {
        return select(QuerydslUtils.record(type, path, exprs));
    }
