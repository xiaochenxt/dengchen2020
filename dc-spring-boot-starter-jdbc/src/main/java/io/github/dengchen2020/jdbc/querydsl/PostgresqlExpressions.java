...
public class PostgresqlExpressions {
    ...
    public static Predicate findInSet(StringPath stringPath, String value) {
        return Expressions.stringTemplate("array_position(string_to_array({0},','),{1})", stringPath, value).isNotNull();
    }
    ...
    public static Predicate jsonContains(StringPath stringPath, Number... value) {
        return Expressions.booleanTemplate("{0} @> {1}::jsonb", stringPath, JsonUtils.toJson(value)).eq(Boolean.TRUE);
    }
    ...
    public static Predicate jsonContains(StringExpression jsonb, String... key) {
        return Expressions.booleanTemplate("'{0} @> {1}::jsonb", jsonb, JsonUtils.toJson(key)).eq(Boolean.TRUE);
    }
    ...
    public static StringTemplate jsonExtractString(StringExpression jsonb, String key){
        return Expressions.stringTemplate("{0} ->> {1}", jsonb, Expressions.asString(key));
    }
    ...
    public static StringTemplate jsonExtract(StringExpression jsonb, int index){
        return Expressions.stringTemplate("{0} -> {1}", jsonb, index);
    }
