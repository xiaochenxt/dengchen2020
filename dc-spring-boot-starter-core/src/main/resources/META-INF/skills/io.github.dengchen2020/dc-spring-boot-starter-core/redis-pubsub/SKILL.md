...
| **序列化** | 默认使用 `GenericJacksonJsonRedisSerializer`，含类型信息 |
...
RedisMessagePublisher.publish(channel, message)
    ↓
serializer.serialize(message)  →  默认 GenericJacksonJsonRedisSerializer（含 @class 类型信息）
    ↓
...
MessageListenerAdapter(bean, methodName)
    ...
    └── 其他类型     → GenericJacksonJsonRedisSerializer
    ↓
...
3. `GenericJacksonJsonRedisSerializer` 带 `@class` 类型信息，反序列化时需保证 class 存在且兼容
