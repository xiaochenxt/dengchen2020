...
public abstract class BeanUtils {
    ...
    private record CacheKey(Class<?> source, Class<?> target, boolean useConverter) {}
    private static final ConcurrentMap<CacheKey, BeanCopier> CACHE = new ConcurrentReferenceHashMap<>();

    @SuppressWarnings("rawtypes")
    private static final ConcurrentMap<CacheKey, RecordCopier> RECORD_CACHE = new ConcurrentReferenceHashMap<>();
    ...
    private static BeanCopier getBeanCopier(Class<?> sourceClass, Class<?> targetClass,@Nullable Converter converter) {
        ...
        return CACHE.computeIfAbsent(cacheKey, key -> BeanCopier.create(key.source, key.target, key.useConverter));
    }
    ...
    private static <T extends Record> T getRecordCopier(Class<?> sourceClass, Class<T> targetClass, Object source,@Nullable Converter converter) {
        boolean useConverter = converter != null;
        var cacheKey = new CacheKey(sourceClass, targetClass, useConverter);
        RecordCopier copier = RECORD_CACHE.computeIfAbsent(cacheKey, key -> RecordCopier.create(key.source, (Class<? extends Record>) key.target, key.useConverter));
