...
| 类型 | 方法 | 说明 |
|------|------|------|
| **算术** | `createArithmetic()` | 数字算术表达式，如 `1+2=?` |
| **中文** | `createChinese()` | 中文汉字验证码 |
| **英文数字** | `createSpec()` | 字母数字组合 |
...
String code = captcha.code();        // 验证码结果（如 "3"）
```
...
```java
CaptchaUtils.Captcha captcha = CaptchaUtils.createSpec();
String base64 = captcha.toBase64();
String code = captcha.code();
```
...
public R<Map<String, String>> captcha() {
    var captcha = CaptchaUtils.createArithmetic();
    String uuid = StrUtils.uuidSimplified();
    // 将验证码结果存入 Redis，key = captcha:uuid，value = code
    redisTemplate.opsForValue().set("captcha:" + uuid, captcha.code(),
            5, TimeUnit.MINUTES);
