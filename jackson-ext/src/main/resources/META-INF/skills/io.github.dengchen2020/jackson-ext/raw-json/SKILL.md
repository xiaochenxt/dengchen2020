...
序列化是 Jackson 自带的默认行为，`@JsonRawValue` 直接使用 Jackson 内置的 `RawSerializer<String>` 输出原生 JSON，不进行转义。

### 反序列化

`JsonRawValueAnnotationIntrospector` 检测到 `@JsonRawValue` 注解后，为其注册 `RawDeserializer`，核心逻辑在 `RawDeserializer.deserialize()`:
